...
```ts
{
  ...
  agentId?: string;        // continue a child via agentId instead of starting a new one
...
After a conversation-mode child answers, it parks instead of terminating. The `<agents>` block in the parent's context lists each parked child's `agentId`. Pass that `agentId` to the same subagent tool with a new `message` to continue the existing child session; omit it to start a new child.

After each subagent result, eve adds a labeled system message containing an `<agents>` block to the parent's history. This block lists the agents the parent can continue via `agentId`, along with their latest status. Context compaction keeps only the newest copy of the block.

The parent holds agent handles only for its session lifetime. When the parent session ends, eve terminates its children. For [remote agents](./guides/remote-agents), both deployments must run the same eve version to use agent messaging.
