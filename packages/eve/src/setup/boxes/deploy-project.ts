...
export interface DeployProjectOptions {
   ...
   * inside `perform` (the former channel setup composition; onboarding always has
 ...
 * former channel setup composition), it falls back to the interactive bare
 ...
 * every deploy surface — the dev TUI `/deploy` and onboarding —
...
export function deployProject(
  ...
  return {
    ...
    async perform({ state, input, signal }): Promise<DeployProjectPayload> {
      ...
      // only place this server-side setting is touched, so a local setup change
