...
import { runRegistrySetupCommandWithPrompter } from "#cli/commands/registry-setup-command.js";
import type {
  Prompter,
  SelectMetadata,
  SelectNotice,
...
const REGISTRY_CATEGORIES: ReadonlyArray<{
...
}> = [
  {
    ...
    prefix: "channel/",
    label: "Add a way to chat",
    hint: "Web Chat, Slack, Discord, Teams…",
  },
  {
...
export type RegistryFlowResult =
  | { kind: "done"; addedItems: readonly string[] }
  | { kind: "cancelled" };
...
function itemRows(items: readonly RegistryCatalogItem[]): SelectOption<RegistryRow>[] {
  return items.map((item, index) => ({
    value: `item:${index}`,
    label: `${itemLabel(item)} · ${item.source}`,
...
function categoryRows(): SelectOption<RegistryRow>[] {
  return [
    ...
    })),
    {
      ...
      hint: "Search the complete registry or enter an item address",
    },
    { value: DONE, label: "Start chatting", trailingAction: true },
  ];
}
...
async function inspectItem(
  ...
  while (true) {
    ...
    const spinner = prompter.log.spinner?.(`Adding ${item.name}…`);
    try {
      await deps.installRegistryItem(
        appRoot,
        item.address,
        {
          silent: false,
          withInheritedStdio: prompter.withInheritedStdio,
        },
        {
          loadSetupCommandRunner: async () => async (root, setup) => {
            spinner?.stop();
            await runRegistrySetupCommandWithPrompter(root, setup, prompter, signal);
          },
        },
      );
    } finally {
  ...
  try {
    while (true) {
      ...
      const selectedCategory = await input.prompter.select<RegistryRow>({
        ...
        description: "Choose a starting point. You can come back anytime with /add.",
      ...
      if (
        ...
        return { kind: "done", addedItems: [resolved.item.address] };
      }
    }
  } catch (error) {
